from drf_api_logger.middleware.api_logger_middleware import *
from ..jwt_auth_module.token_handler import AuthJwt




class LoggerMiddleware(APILoggerMiddleware):
    ...
    def __call__(self, request):
        ...
        if self.DRF_API_LOGGER_DATABASE or self.DRF_API_LOGGER_SIGNAL:
            ...
            if auth_header:
                ...
                if len(splitted_auth) == 2 and splitted_auth[0] == 'Bearer':
                    token = splitted_auth[1]
                    jwt_handler = AuthJwt()
                    validated_token = jwt_handler.get_validated_token(token)
                    user = jwt_handler.get_user_from_token(validated_token)
                    if user:
                        headers['users']={
