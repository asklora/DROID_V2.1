...
def write_to_sql(predicted_values, indices_df, args):
    ...
    columns = ['data_period', 'when_created', 'forward_date', 'spot_date', 'year', 'week', 'day_of_week',
               'num_periods_to_predict',
               'number_of_quantiles', 'index', 'ticker',
    ...
    for i in range(stock_list.shape[0]):
        if sum(indices_df.ticker == stock_list.iloc[i][0]) > 0:
            to_aws_df.loc[i, 'index'] = indices_df[indices_df.ticker == stock_list.iloc[i][0]]['index'].iloc[0]
...
def write_to_sql_model_data(args):
    ...
    if args.production_output_flag:  # if writing to production_data table, i.e., write out the stocks - keep in "live"
        ...
        engine = create_engine(global_vars.DB_PROD_URL_WRITE, pool_size=cpu_count(), max_overflow=-1, isolation_level="AUTOCOMMIT")
