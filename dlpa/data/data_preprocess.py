...
def create_beta_df(full_df2, indices_df2, args):
    ...
    def bet_pool(data_inputs):
        ...
        for col in a.columns:
            ...
            i = 1
            while (a.index[-1] + BDay(i) <= beta_df.index[-1]) & (i < 5):
                beta_df.loc[a.index[-1] + BDay(i), col] = beta_df.loc[a.index[-1], col]
                i = i + 1

    for i in range(len(weekly_ret) - (52 * 5)):
        a = weekly_ret.loc[weekly_ret[i:i + (52 * 5) + 1].index, :]
        print(a.index[-1])
        if a.index[-1].weekday() == 4:
