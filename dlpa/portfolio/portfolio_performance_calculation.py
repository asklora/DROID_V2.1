...
def performance_calculator(df):
    ...
    optimized_values = df.groupby(["currency_code","signal_threshold","topX_multiplier"]).agg({"wts_performance": ["mean"], "bench_performance": ["mean"],
                                                       "direction": ["sum", "count"],
                                                       "out_perf_direction": ["sum", "count"]})
    # optimized_values.reset_index(inplace=True)

    optimized_values2 = df.groupby(["currency_code","spot_date","signal_threshold","topX_multiplier"]).agg({"wts_tri_performance": ["mean"],
                                                  "bench_tri_performance": ["mean"]})
    optimized_values2.reset_index(inplace=True)
    optimized_values2 = optimized_values2.groupby([("currency_code",""),("signal_threshold",""),("topX_multiplier","")]).agg({("wts_tri_performance","mean"): ["prod","std"],
