...
from general.sql_query import read_query
import global_vars
from global_vars import no_top_models, portfolio_period, signal_threshold
from general.table_name import get_top_stock_models_stock_table_name, get_top_stock_models_table_name

def portfolio_maker(portfolio_period, forward_date=None, spot_date=None, live=False):
    ...
    if live:
        ...
        if portfolio_period == 0:
            while d.weekday() != 4:
                d += BDay(1)
            forward_date = d
        elif portfolio_period == 1:
            if d.weekday() > 4:
                while d.weekday() != 0:
                    d += BDay(1)
            forward_date = d
    else:
        if forward_date is None and spot_date is None:
            sys.exit("Please specify either of the forward_date or spot_date!")
        if forward_date is not None and spot_date is not None:
            sys.exit("Please specify only forward_date or spot_date!")

        if forward_date is not None:
            forward_date = datetime.strptime(f"{forward_date}", "%Y-%m-%d").date()

        if spot_date is not None:
            spot_date = datetime.strptime(f"{spot_date}", "%Y-%m-%d").date()

            if portfolio_period == 0:
                forward_date = spot_date + Week(1)
            elif portfolio_period == 1:
                forward_date = spot_date + BDay(1)
    ...
    # *************************************************************************************************
    # ****************************** Download the models **********************************************
    query = f"select * from {model_table_name} where data_period = '{period}' and forward_date = '{forward_date_0}' and num_periods_to_predict = {num_periods_to_predict}"
    models_df = read_query(query, model_table_name, cpu_counts=True)
    models_df = models_df[["data_period", "when_created", "forward_date", "best_train_acc", "best_valid_acc", "model_type", "model_filename", "pc_number"]]
    if len(models_df) == 0:
        sys.exit(str(
            "We don't have inferred data for the %s with data period %s." % (forward_date, portfolio_period)))
    models_df.fillna(value=0, inplace=True)
    # *************************************************************************************************
    # ****************************** Download stock data **********************************************
    query = f"select * from {stock_table_name} where data_period = '{period}' and forward_date = '{forward_date_1}'"
    full_df = read_query(query, stock_table_name, cpu_counts=True)
    full_df = full_df[["data_period", "when_created", "forward_date", "spot_date", "currency_code", "ticker", "predicted_quantile_1", "signal_strength_1", "model_filename"]]
    if len(full_df) == 0:
        sys.exit(str(
            "We don't have inferred data for the %s with data period %s." % (forward_date, portfolio_period)))
    ...
    # *************************************************************************************************
    full_df = full_df[full_df.signal_strength_1 > full_df.signal_strength_1.quantile(1 - signal_threshold)]

    gb = full_df.groupby(["data_period", "forward_date", "currency_code", "ticker", "predicted_quantile_1", "spot_date"])
    portfolio_1 = gb.size().to_frame(name="counts")
    portfolio_1 = (portfolio_1.join(gb.agg({"signal_strength_1": "mean"}).rename(columns={"signal_strength_1": "signal_strength_mean"})).reset_index())
