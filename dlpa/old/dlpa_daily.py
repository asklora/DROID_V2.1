...
from data.data_preprocess import dataset, dataset_prep, create_train_test_valid, create_train_test_valid_ohlcv
...
parser.add_argument('--forward_week', type=int, default=1, help='The week of the test date.')
parser.add_argument('--forward_day', type=int, default=1, help='The day of the test date.')
parser.add_argument('--stock_percentage', type=float, default=1.0,
                    help='The percentage of the stocks to use in the whole data.')
# *******************  DATASET PARAMETERS  *************************************
# ******************************************************************************
parser.add_argument('--data_period', type=int, default=0, help='data period, 0 = weekly or 1 = daily')
parser.add_argument('--train_num', type=int, default=260, help='Number of training periods.')
parser.add_argument('--valid_num', type=int, default=5, help='Number of validation periods.')
parser.add_argument('--test_num', type=int, default=5, help='Number of test periods.')

parser.add_argument('--dow', type=int, default=4, help='dow=day of week, monday->0, friday->4')
parser.add_argument('--candle', type=str, default='close', help='The candle used for X or Y.')
parser.add_argument('--data_type', type=str, default='X', help='X or Y.')
parser.add_argument('--all_candle', type=bool, default=True, help='Use all candles for X.')

parser.add_argument('--period_jump', type=int, default=6,
...
# *******************  DATA PARAMETERS  ****************************************
# ******************************************************************************
parser.add_argument('--lookback', type=int, default=10, help='Number of lookbacks for each data input')
parser.add_argument('--num_periods_to_predict', type=int, default=1, help='Number of weeks or days to predict.')
...
parser.add_argument('--test_table_name', type=str, default='DLPA_test_daily', help='The test table name in AWS.')
parser.add_argument('--production_table_name', type=str, default='DLPA_production_daily',
                    help='The production table name in AWS.')
parser.add_argument('--model_data_table_name', type=str, default='DLPA_model_data_daily',
...
def main(args):
    ...
    args.aws_columns_list = ['start_year', 'start_week', 'forward_year', 'forward_week', 'forward_day',
                             'stock_percentage',
                             'valid_num', 'test_num',
                             'lookback', 'num_weeks_to_predict', 'num_bins', 'num_nans_to_skip',
                             'accuracy_for_embedding', 'epoch',
                             ...
                             'best_valid_epoch', 'best_train_epoch', 'timestamp', 'model_run_time',
                             'pc_number', 'train_num']
    ...
    # len(full_df) - full_df['volume'].count()

    if platform.system() == 'Linux':
        args.model_path = '/home/loratech/PycharmProjects/models/dlpa_daily/'
        if not os.path.exists(args.model_path):
            os.makedirs(args.model_path)
        args.plot_path = '/home/loratech/PycharmProjects/plots/dlpa_daily/'
        if not os.path.exists(args.plot_path):
            os.makedirs(args.plot_path)
    else:
        args.model_path = 'C:/dlpa_master/model/dlpa_daily/'
        Path(args.model_path).mkdir(parents=True, exist_ok=True)
        args.plot_path = 'C:/dlpa_master/plots/dlpa_daily/'
    ...
    def hyper_fn(hypers):
        ...
        # dow=day of week, "0"->Mon or "1"->Tue or "2"->Wed or "3"->Thu or "4"->Fri
        args.dow = 2
        # args.candle = 'close'
        # args.data_type = 'X'
        # args.all_candle = True

        # 0 -> weekly or 1 -> daily
        args.period = 0

        # 0 -> X_Candles or 1 -> X_returns or 2 -> Y_returns
        args.data_type = 1

        # 0 -> all, 1 -> open, 2 -> high, 3 -> low, 4 -> close, 5 -> volume
        args.candle_type = 0

        candles_X, returns_X, returns_Y = dataset(datasett, args)  # Candles, the second one is returns

        trainX, trainY, validX, validY, testX, testY, final_prediction = create_train_test_valid(returns_X,
                                                                                                 returns_Y,
                                                                                                 args)
...
if __name__ == "__main__":
    ...
    if args_main.data_period == 0:
        d0 = timedelta(weeks=args_main.train_num + args_main.valid_num + 1)
    else:
        d0 = timedelta(days=args_main.train_num + args_main.valid_num + 1)

    t2_1 = year_week_day(args_main.forward_year, args_main.forward_week, args_main.forward_day)
    t2_2 = year_week_day(args_main.forward_year, args_main.forward_week, args_main.forward_day)

    u0 = t2_1 - d0
    args_main.start_year, args_main.start_week, args_main.start_day = datetime.date(u0).isocalendar()
    ...
    while flag:
        ...
        t1 = year_week_day(args_main.start_year, args_main.start_week, args_main.start_day)

        # Needed for making the test date as anchor (only for end_date)
        if args_main.data_period == 0:
            d2 = timedelta(weeks=args_main.test_num)
        else:
            d2 = timedelta(days=args_main.test_num)
        ...
        args_main.start_year, args_main.start_week, args_main.start_day = datetime.date(u1).isocalendar()
        args_main.end_year, args_main.end_week, args_main.end_day = datetime.date(u2_2).isocalendar()
        ...
        if args_main.data_period == 0:
            d = timedelta(weeks=args_main.period_jump)
        else:
            d = timedelta(days=args_main.period_jump)
