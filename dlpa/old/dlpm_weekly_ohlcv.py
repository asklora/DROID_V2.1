...
from model.old.model_dlpm_weekly_ohlcv import full_model
...
parser.add_argument('--num_kern', type=int, default=12)
parser.add_argument('--layers', type=int, default=2)
parser.add_argument('--gru_nodes', type=int, default=4)
parser.add_argument('--gru_nodes_mult', type=int, default=0)
parser.add_argument('--wr_layers', type=int, default=2)
parser.add_argument('--wr_gru_nodes', type=int, default=8)
parser.add_argument('--wr_gru_nodes_mult', type=int, default=0)
parser.add_argument('--gru_drop', type=float, default=0.5)
parser.add_argument('--wr_gru_drop', type=float, default=0.5)
...
parser.add_argument('--test_table_name', type=str, default='DLPM_forward_weekly', help='The test table name in AWS.')
parser.add_argument('--production_table_name', type=str, default='DLPM_production_weekly',
                    help='The production table name in AWS.')
parser.add_argument('--model_data_table_name', type=str, default='DLPM_model_data_weekly',
...
def main(args):
    space = {
        'num_kern': hp.choice('num_kern', [32, 64, 128, 256]),
        ...
        'gru_drop': hp.quniform('gru_drop', 0, 0.5, 0.25),
        'wr_gru_drop': hp.quniform('wr_gru_drop', 0, 0.5, 0.25),
        'Lookback': hp.choice('Lookback', [10, 20]),
    }
    ...
    args.aws_columns_list = ['start_year', 'start_week', 'stock_percentage', 'valid_num', 'test_num', 'lookback',
                             ...
                             'batch_size', 'lr',
                             'num_kern', 'layers', 'gru_nodes', 'gru_nodes_mult', 'wr_layers', 'wr_gru_nodes',
                             'wr_gru_nodes_mult', 'gru_drop', 'wr_gru_drop',
    ...
    if platform.system() == 'Linux':
        args.model_path = '/home/loratech/PycharmProjects/models/dlpm_weekly_ohlcv/'
        if not os.path.exists(args.model_path):
            os.makedirs(args.model_path)
        args.plot_path = '/home/loratech/PycharmProjects/plots/dlpm_weekly_ohlcv/'
        if not os.path.exists(args.plot_path):
            os.makedirs(args.plot_path)
    else:
        args.model_path = 'C:/dlpa_master/model/dlpm_weekly_ohlcv/'
        Path(args.model_path).mkdir(parents=True, exist_ok=True)
        args.plot_path = 'C:/dlpa_master/plots/dlpm_weekly_ohlcv/'
        Path(args.plot_path).mkdir(parents=True, exist_ok=True)

    def hyper_fn(hypers):
        # Hyperopt function which optimizes the whole process.
        args.num_kern = int(hypers['num_kern'])
        args.lr = float(hypers['Learning_rate'])
        args.batch_size = 2 ** int(hypers['batch_size'])
        args.layers = int(hypers['layers'])
        args.gru_nodes = int(hypers['gru_nodes'])
        args.gru_nodes_mult = float(hypers['gru_nodes_mult'])
        args.wr_layers = int(hypers['wr_layers'])
        args.wr_gru_nodes = int(hypers['wr_gru_nodes'])
        args.wr_gru_nodes_mult = int(hypers['wr_gru_nodes_mult'])
        args.gru_drop = float(hypers['gru_drop'])
        args.wr_gru_drop = float(hypers['wr_gru_drop'])
        ...
        del trainX1, trainX2, trainY, validX1, validX2, validY, testX1, testX2, testY
        gc.collect()
