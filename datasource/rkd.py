...
def bulk_update_rtdb(data):
    ...
    for ticker, val in data.items():
        if not ticker == "price":
            ref = db.collection(
                settings.FIREBASE_COLLECTION["universe"]
            ).document(ticker)
...
class Rkd:
    ...
    def send_request(self, url, payload, headers):
        ...
        try:
            ...
            error_log = ErrorLog.objects.create_log(
                error_description="TRKD REQUEST ERROR", error_message=str(e)
            )
    ...
    def get_token(self):
        ...
        if response:
            self.credentials.token = response["CreateServiceToken_Response_1"][
                "Token"
            ]
    ...
    def auth_headers(self):
        ...
        headers["X-Trkd-Auth-ApplicationID"] = self.credentials.extra_data[
            "key"
        ]
        headers["X-Trkd-Auth-Token"] = self.credentials.token
        return headers
...
class RkdData(Rkd):
    ...
    def get_index_price(self, currency: str):
        ...
        payload = self.retrive_template(
            currency.index_ticker, fields=["CF_CLOSE"]
        )
    ...
    def get_quote(self, ticker, df=False, save=False, **options):
        ...
        if save:
            ...
            if "detail" in options:
                ...
                collected_data["hedge_uid"] = (
                    collected_data["ticker"].astype(str) + options["detail"]
                )
                collected_data["hedge_uid"] = (
                    collected_data["hedge_uid"]
                ...
                self.save(
                    "master",
                    "HedgeLatestPriceHistory",
                    collected_data.to_dict("records"),
                )
            print("saving done")
...
class RkdStream(RkdData):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.ticker_data:
            ...
            self.ticker_data = [
                data["ticker"]
                for data in Universe.objects.filter(
                    mic__in=exchange, is_active=True
                ).values("ticker")
            ]
    ...
    def stream_quote(self):
        ...
        if not settings.RUN_LOCAL:
            while True:
                open_market = ExchangeMarket.objects.filter(
                    currency_code__in=["HKD", "USD"], group="Core", is_open=True
                ).values_list("currency_code", flat=True)
                # usd_exchange,hkd_exchange =ExchangeMarket.objects.filter(mic='XNAS'),ExchangeMarket.objects.get(mic='XHKG')
                if open_market:
                    self.ticker_data = list(
                        Universe.objects.filter(
                            currency_code__in=open_market, is_active=True
                        )
    ...
    def get_list_exchange(self):
        if not self.exchange_list:
            self.exchange_list = [
                exc["mic"]
                for exc in ExchangeMarket.objects.filter(
                    currency_code__in=["HKD", "USD"], is_open=True
                ).values("mic")
            ]
    ...
    def answer_ping(self, ws, *args, **options):
        ...
        if self.exchange_list:
            ...
            print(
                json.dumps(
                    ping_req, sort_keys=True, indent=2, separators=(",", ":")
                )
            )
            if current_live_exchange != len(self.exchange_list):
                ...
                self.ticker_data = [
                    data["ticker"]
                    for data in Universe.objects.filter(
                        mic__in=self.exchange_list, is_active=True
                    ).values("ticker")
                ]
    ...
    def write_on_s3(self, message, *args, **options):
        try:
            ...
            s3_file = (
                "test_dir/" + type_name + "-" + ticker + "-" + act + ".txt"
            )
    ...
    def update_rtdb(self, data):
        ...
        logging.info(ticker)
        ref = db.collection(settings.FIREBASE_COLLECTION["universe"]).document(
            ticker
        )
        try:
