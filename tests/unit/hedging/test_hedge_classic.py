...
import pytest
from bot.factory.act.creator import ClassicCreator
...
def test_create_bot() -> None:
    ...
    assert result.get("vol") == 0.47720638442222607
    assert result.get("classic_vol") == 0.561546542852443

    # check if the resulting class is correct
    assert isinstance(res, ClassicCreator)
