...
def bench_fn(args):
    ...
    def statistic_calculator(df):
        ...
        # bench_df.fillna({'avg_days_ko': 0}, inplace=True)
    ...
    #
    #     query = table0.delete().where(table0.columns.bot_type == 'UCDC')
    #     conn.execute(query)
    ...
    # engine.dispose()
    report_to_slack("{} : === Executive ucdc statistics monthly completed ===".format(str(dt.now())), args)

    print(f'Saved the data to {table_name} for UCDC !')
