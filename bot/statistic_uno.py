...
def populate_uno_statistic(ticker=None, currency_code=None, time_to_exp=None):
    ...
    def statistic_calculator(df):
        ...
        days_ko = df.groupby(["ticker", "time_to_exp", "event"]).agg({"duration_days": ["mean"]}).rename(columns=d)
        ...
        days_max_loss_pos = df.groupby(["ticker","time_to_exp"])["duration_days"].agg([("pct_max_loss_flag", lambda x: x[x > 0].mean())])
        ...
        bench_df["pct_max_profit"] = returns["KO_num", "sum"] / returns["bot_return", "count"]
        ...
        bench_df = bench_df.merge(days_ko[days_ko.event == "KO"], on=["ticker","time_to_exp"], how="outer")
        ...
        bench_df["pct_max_loss"] = returns["pct_max_loss_flag", "sum"] / returns["bot_return", "count"]
