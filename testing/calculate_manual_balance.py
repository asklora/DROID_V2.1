...
if __name__ == "__main__":
    ...
    balance = read_query(query, prints=True)
    print(balance)
    result = pd.DataFrame({"outcome2" : [] , "user_id" : []}, index=[])
    for index, row in balance.iterrows():
        user_id = row["user_id"]

        query = f"select coalesce(amount, 0) as outcome2 from user_transaction ut "
        ...
        first_deposit2 = read_query(query, prints=True)
        print(first_deposit2)

        query = f"select coalesce(sum(investment_amount), 0)  as outcome2 from orders_position where user_id = {user_id} and is_live = False;"
        total_investment_false = read_query(query, prints=True)
        ...
        total_return = read_query(query, prints=True)
        print(total_return)
        ...
        total_fee_false = read_query(query, prints=True)
        ...
        total_investment_true = read_query(query, prints=True)
        ...
        total_fee_true = read_query(query, prints=True)
        ...
        # query += f"where ut.balance_uid = (select balance_uid from user_account_balance where user_id = {user_id}) and "
        # query += f"transaction_detail ->> 'event' ='first deposit'"
        # first_deposit = read_query(query, prints=True).loc[0, "outcome"]
        # print(first_deposit)
        ...
        # total_debit_false = read_query(query, prints=True)
        ...
        # total_credit_false = read_query(query, prints=True)
        ...
        # total_debit_true = read_query(query, prints=True)
