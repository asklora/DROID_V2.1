...
def read_query(query, table=get_universe_table_name(), cpu_counts=False, dlp=False, alibaba=False, prints=True):
    if(prints):
...
def get_active_universe_consolidated_by_field(isin=False, cusip=False, sedol=False, manual=False, ticker=None):
    query = f"select * from {get_universe_consolidated_table_name()} where is_active=True "
    if isin:
