...
droid2 = "postgres://postgres:ml2021#LORA@droid-v2-production-cluster.cluster-cy4dofwtnffp.ap-east-1.rds.amazonaws.com:5432/postgres"
...
def upsert_data_to_db(index, index_type, data, table_name, method="update"):
    ...
    engine = create_engine(droid2, max_overflow=-1,
...
def insert_data_to_database(db_url, data, table):
    ...
    engines = create_engine(db_url, max_overflow=-1,
                            isolation_level="AUTOCOMMIT")
    try:
...
def insert_data_to_database_manually(db_url, field, data, table, index):
    engine_prod = create_engine(
        db_url, max_overflow=-1, isolation_level="AUTOCOMMIT")
...
def get_data_from_database_condition(db_url, table, condition):
    ...
    engine = create_engine(db_url, max_overflow=-1,
                           isolation_level="AUTOCOMMIT")
...
def data_dividend():
    ...
    #data = get_data_from_database(droid, table)
    data = get_data_from_database_condition(
        droid, table, f" ticker in {get_ticker_from_new_droid()} ")
    data = data[column]
...
def data_universe_detail():
    initial_data = get_data_from_database_condition(
        droid2, "universe", f" ticker in {get_ticker_from_new_droid()} ")
    initial_data = initial_data.drop(columns=["quandl_symbol", "ticker_name", "ticker_fullname", "lot_size", "entity_type",
                                              "industry_code", "wc_industry_code", "company_description", "fiscal_year_end", "icb_code", "worldscope_identifier"])
    print(initial_data)
    table = "droid_universe"
    column = ["ticker", "quandl_symbol", "ticker_name", "ticker_fullname",
              "lot_size", "entity_type", "industry_code", "wc_industry_code"]
    data = get_data_from_database_condition(
        droid, table, f" ticker in {get_ticker_from_new_droid()} ")
    ...
    data2 = get_data_from_database_condition(
        droid, table, f" ticker in {get_ticker_from_new_droid()} ")
    ...
    data3 = get_data_from_database_condition(
        dlp, table, f" ticker in {get_ticker_from_new_droid()} ")
...
def currency_calendar():
    ...
    print(data)
    data["currency_code"] = np.where(
        data["currency_code"] == "KR", "KRW", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "US", "USD", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "HK", "HKD", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "SG", "SGD", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "CN", "CNY", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "TW", "TWD", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "JP", "JPY", data["currency_code"])
    data["currency_code"] = np.where(
        data["currency_code"] == "GB", "GBP", data["currency_code"])
    data["currency_code"] = np.where(~data["currency_code"].isin(
        ["KRW", "USD", "HKD", "SGD", "CNY", "TWD", "JPY", "GBP"]), "EUR", data["currency_code"])
    data = uid_maker(data, trading_day="non_working_day",
...
def industry_group():
    ...
    print(data)
    # insert_data_to_database(droid2, data, table)
    upsert_data_to_db("industry_group_code", TEXT,
                      data, table, method="update")
    print(f"Get {table} = True")
...
def currency_member_type():
    ...
    data = pd.DataFrame({"member_type_id": [1, 2, 3], "type_description": [
                        "Retrieve Member", "Custom Member", "User Defined Member"]}, index=[0, 1, 2])
    print(data)
    # insert_data_to_database(droid2, data, table)
...
def currency():
    ...
    data = data.rename(
        columns={'vix_index': 'vix_id', 'region_code': 'region_id'})
    print(data)
    data = data[column]
    data["is_active"] = True
    data = data.drop_duplicates(
        subset=["currency_code"], keep="first", inplace=False)
...
def universe_consolidated():
    ...
    SPX = get_data_from_database_condition(
        dlp, "universe", "is_active=True and index_id='0#.SPX'")
    consolidated_spx = get_data_from_database(dlp, "consolidated_ticker")
    SPX = SPX.merge(consolidated_spx, how="left", left_on=[
                    "ticker"], right_on=["consolidated_ticker"])
    ...
    SPX = SPX[["consolidated_ticker", "origin_ticker",
               "isin", "still_live", "updated"]]
    SPX["is_consolidated"] = True
    SPX["has_isin"] = True
    print(SPX)

    retrieve_ticker = get_data_from_database_condition(
        dlp, "universe", "is_active=True and index_id not in ('0#.SPX')")
    retrieve_ticker = retrieve_ticker.rename(columns={'ticker': 'consolidated_ticker',
                                                      'is_active': 'still_live',
                                                      'last_updated': 'updated'})
    retrieve_ticker = retrieve_ticker[[
        "consolidated_ticker", "still_live", "updated"]]
    ...
    data = data[column]
    data = data.drop_duplicates(
        subset=["consolidated_ticker"], keep="first", inplace=False)
    print(data)
    # insert_data_to_database(droid2, data, table)
    upsert_data_to_db("consolidated_ticker", TEXT,
                      data, table, method="update")
    print(f"Get {table} = True")
    sys.exit(1)
...
def universe_excluded():
    ...
    excluded_tickers = excluded_tickers.rename(
        columns={'excluded_ticker': 'ticker'})
    excluded_tickers = excluded_tickers["ticker"]
    print(excluded_tickers)

    for ticker in excluded_tickers:
        insert_data_to_database_manually(droid2, tuple(
            column), (ticker, True, False), table, index)
    ...
    for ticker in dsws_excluded_ticker:
        insert_data_to_database_manually(droid2, tuple(
            column), (ticker, False, True), table, index)
...
def universe_rating():
    ...
    data = get_data_from_database_condition(
        droid, "droid_universe", f"is_active=True and ticker in {get_ticker_from_new_droid()} ")
    data = data[column]
    print(data)
...
def data_dss():
    ...
    table = "data_dss"
    data = get_data_from_database_condition(
        dlp, "dss_data", f"ticker in {get_ticker_from_new_droid()} and trading_day >='{start_date}'")
...
def data_dsws():
    ...
    table = "data_dsws"
    data = get_data_from_database_condition(
        dlp, "dsws_data", f"ticker in {get_ticker_from_new_droid()} and trading_day >='{start_date}'")
...
def data_quandl():
    ...
    table = "data_quandl"
    data = get_data_from_database_condition(
        droid, "droid_quandl_orats_vols", f"ticker in {get_ticker_from_new_droid()}  and trading_day >='{start_date}'")
    data = data[column]
    print(data)
...
def data_fundamental_score():
    ...
    data = get_data_from_database_condition(
        droid, "fundamentals_score", f"ticker in {get_ticker_from_new_droid()}")
    data = data[column]
    print(data)
    upsert_data_to_db("ticker", TEXT, data, table, method="update")
    print(f"Get {table} = True")
...
def data_ibes():
    ...
    table = "data_ibes"
    data = get_data_from_database_condition(
        dlp, "ibes_data", f" ticker in {get_ticker_from_new_droid()} ")
    data = uid_maker(data, uid="uid", ticker="ticker",
                     trading_day="period_end", date=True)
    data = data[column]
    print(data)
    upsert_data_to_db("uid", TEXT, data, table, method="update")
    print(f"Get {table} = True")
...
def data_ibes_monthly():
    ...
    data = get_data_from_database_condition(
        dlp, "ibes_data_monthly", f" ticker in {get_ticker_from_new_droid()} ")
    data = uid_maker(data, uid="uid", ticker="ticker",
                     trading_day="trading_day", date=True)
    data = data[column]
    print(data)
    upsert_data_to_db("uid", TEXT, data, table, method="update")
    print(f"Get {table} = True")
...
def data_vol_surface_inferred():
    ...
    data = get_data_from_database_condition(
        droid, "droid_vol_surface_parameters_inferred", f" ticker in {get_ticker_from_new_droid()} ")
    data = uid_maker(data, uid="uid", ticker="ticker",
                     trading_day="trading_day", date=True)
    data = data[column]
    print(data)
...
def latest_price():
    ...
    table = "latest_price"
    data = get_data_from_database_condition(
        droid, "latest_price_updates", f" ticker in {get_ticker_from_new_droid()} ")
    data = data[column]
    print(data)
    upsert_data_to_db("ticker", TEXT, data, table, method="update")
    print(f"Get {table} = True")
...
def data_interest():
    column = ["ticker_interest", "rate", "raw_data", "days_to_maturity",
              "ingestion_field", "maturity", "updated", "currency_code"]
    table = "data_interest"
    data = get_data_from_database(droid, "interest_rate")
    data = data.rename(columns={"ticker": "ticker_interest",
                                "currency": "currency_code", "update_date": "updated"})
    data = data[column]
    print(data)
...
def data_worldscope_summary():
    ...
    data = get_data_from_database_condition(
        dlp, "worldscope_quarter_summary", f" period_end>='{start_date}'::date - interval '3 months' and ticker in {get_ticker_from_new_droid()} ")
