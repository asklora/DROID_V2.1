...
class TradingHours:
    ...
    def get_weekly_holiday(self):
        ...
        url = (
            "https://api.tradinghours.com/v3/markets/holidays?fin_id="
            + fin_id
            + "&start="
            + start_date
            + "&end="
    ...
    def run_market_check(self):
        ...
        if req.status_code == 200:
            ...
            try:
                ...
                local_time_extend = self.normalize_datetime(
                    resp["data"][self.fin_id]["next_bell"]
                )
                ...
                self.until = self.until + timedelta(minutes=15)
                self.next_bell = self.next_bell + timedelta(minutes=15)
            ...
            if self.time_to_check:
                ...
                self.exchange.save()
                slack.report_to_slack(
                    f"===== market check updated {self.exchange.mic} - next in UTC {self.exchange.until_time} , response: {req.status_code}  - market status : {'Open' if self.exchange.is_open else 'Close'} - Reason : {reason} =====",
                    channel="#market_report",
                )

        else:
            slack.report_to_slack(
                f"===== error market check {self.exchange.mic} , {req.status_code} =====",
                channel="#error-log",
            )
    ...
    def is_open(self):
        ...
        if req.status_code == 200:
            resp = req.json()
            try:
                nextbell_time_extend = self.normalize_datetime(
                    resp["data"][self.fin_id]["next_bell"]
                ) + timedelta(minutes=30)
                self.next_bell = self.timezone_to_utc(
                    nextbell_time_extend, self.market_timezone
                )
            except Exception:
                pass
