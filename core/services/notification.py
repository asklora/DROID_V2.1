...
def send_winner_email():
    ...
    winners = list(SeasonHistory.objects.filter(season_id=last_season, 
    rank__gt=0).order_by('rank').values(
        'user_id__email','rank',
    ...
    app.send_task("config.celery.listener",args=(payload,),queue=settings.ASKLORA_QUEUE)