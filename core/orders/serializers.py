    ...
    def side_validation(self, validated_data):
        if validated_data["side"] == "sell":
            ...
            if is_portfolio_exist(
                ...
                raise exceptions.NotAcceptable(
                    {
                        "detail": f"user already has position for {validated_data['ticker']} in current options"
                    }
                )
    ...
    def create(self, validated_data):
        ...
        with db_transaction.atomic():
            if validated_data["side"] == "buy":
                ...
                try:
                    position, order = sell_position_service(
                    ...
                    raise exceptions.NotFound(
                        {"detail": "live position not found error"}
                    )
...
class OrderActionSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        task = order_executor.apply_async(
            args=(payload,), task_id=validated_data["order_uid"]
        )
        data = {
            "action_id": task.id,
            "status": "executed",
