...
def calculate_fee(amount, side, user_id):
    try:
        ...
        stamp_duty_sell = user_client.stamp_duty_sell
        stamp_duty_buy = user_client.stamp_duty_buy
        commissions_sell = user_client.client.commissions_sell
        commissions_buy = user_client.client.commissions_buy
...
def generate_hedge_setup(instance: Order) -> dict:
    ...
    if bot.bot_type.bot_type == "CLASSIC":
        ...
        setup = get_uno(instance.ticker.ticker, instance.ticker.currency_code.currency_code, expiry,instance.created, bot.time_to_exp, instance.amount, instance.price, bot.bot_option_type, bot.bot_type.bot_type, margin=margin)
    elif bot.bot_type.bot_type == "UCDC":
        setup = get_ucdc(instance.ticker.ticker, instance.ticker.currency_code.currency_code, expiry,instance.created, bot.time_to_exp, instance.amount, instance.price, bot.bot_option_type, bot.bot_type.bot_type, margin=margin)
    return setup
...
def order_signal(sender, instance, created, **kwargs):
    ...
    if created and instance.is_init:
        ...
        pass
    #When Order Cancel Bt the Broker, Amount will be back to Balance
        ...
        order_fee = OrderFee.objects.filter(side="debit", transaction_detail__order_uid=str(instance.order_uid))
        if trans.exists():
            trans = trans.get()
            trans.delete()
        if order_fee.exists():
    ...
    #When Populate Position, Order Already Placed But The Status Pending
    elif not created and instance.side == "buy" and instance.status in ["pending"] and instance.is_init and not performance_exist:
        ...
        if not instance.order_type:
            create_fee(instance, instance.user_id, instance.user_id.wallet, None, instance.side)

    #When Populate Position, Order Already Placed But The Status Change to Filled, The Order Will Start Populate Position & Performance
    elif not created and instance.status in "filled" and not performance_exist:
        bot = BotOptionType.objects.get(bot_id=instance.bot_id)
        ...
        if instance.is_init:
            ...
            perf.current_investment_amount = round(perf.last_live_price * perf.share_num, digits)
            perf.current_pnl_ret = (perf.current_bot_cash_balance + perf.current_investment_amount - order.investment_amount) / order.investment_amount
    ...
    # elif not created and instance.status in "filled" and performance_exist:
            ...
            if not bot.is_stock():
                ...
                if instance.setup:
                    ...
                    order_position = OrderPosition.objects.get(position_uid=instance.setup["position"]["position_uid"])
                        ...
                        if not key in ["created", "updated", "share_num"]:
                            if hasattr(order_position, key):
                                ...
                                if field.one_to_many or field.many_to_many or field.many_to_one or field.one_to_one:
                                    raw_key = f"{key}_id"
                                    setattr(order_position, raw_key, instance.setup["position"].pop(key))
                                else:
                                    setattr(order_position, key, instance.setup["position"].pop(key))
                    ...
                    if instance.side == "sell" and order_position.is_live and not instance.order_type:
                        create_fee(instance, order_position.user_id, order_position.user_id.wallet, order_position.position_uid, "sell")
                        ...
                        create_fee(instance, order_position.user_id, order_position.user_id.wallet, order_position.position_uid, "buy")
