...
def firebase_user_delete():
    ...
    firebase_delete = firebase_portfolio.loc[
        ~firebase_portfolio["user_id"].isin(user)
    ]["user_id"].to_list()
...
def firebase_universe_delete():
    ...
    active_universe = universe.loc[universe["is_active"] == True][
        "ticker"
    ].to_list()

    firebase_universe = get_all_universe_from_firestore()
    firebase_delete = firebase_universe.loc[
        ~firebase_universe["ticker"].isin(active_universe)
    ]["ticker"].to_list()
...
def factor_column_name_changes():
    ...
    name_df["index"] = np.where(
        name_df["scaler"].notnull(),
        [
            f"{x}_{y}_currency_code"
            for x, y in name_df[["name", "scaler"]].to_numpy()
        ],
        name_df["name"].values,
    )
...
@log2es("ai_score")
def firebase_universe_update(
    ticker=None, currency_code=None, update_firebase=True
):
    ...
    # Populate Universe
    all_universe = get_active_universe(
        ticker=ticker, currency_code=currency_code
    )
    currency = get_active_currency(currency_code=currency_code)

    all_universe = all_universe.loc[
        ~all_universe["ticker"].isin(currency["index_ticker"].to_list())
    ]
    all_universe = all_universe.loc[
        ~all_universe["ticker"].isin(currency["etf_ticker"].to_list())
    ]
    ...
    for tick in universe["ticker"].unique():
        ...
        details = pd.DataFrame(
            {"ticker": [tick], "detail": [detail_data[0]]}, index=[0]
        )
    ...
    latest_price = get_latest_price_data(
        ticker=ticker, currency_code=currency_code
    )
    ...
    for tick in latest_price["ticker"].unique():
        ...
        price = pd.DataFrame(
            {"ticker": [tick], "price": [price_data[0]]}, index=[0]
        )
    ...
    universe_rating = get_universe_rating_history(
        ticker=ticker, currency_code=currency_code
    )
    universe_rating_detail = get_universe_rating_detail_history(
        ticker=ticker, currency_code=currency_code
    )
    universe_rating_detail = universe_rating_detail.merge(
        universe_rating[["ticker", "dlp_1m", "wts_rating"]],
        how="left",
        on=["ticker"],
    )

    universe_rating_positive_negative = pd.DataFrame(
        {"ticker": [], "positive_factor": [], "negative_factor": []}, index=[]
    )
    ...
    for (
        ...
        factor_use.items()
    ):  # work on different currency separately -> can use different factors
        lst = [x for x in ",".join(v).split(",") if len(x) > 0]
        lst = list(set(lst))
        lst += [x + "_minmax_currency_code" for x in lst]

        # select dataframe for given industry
        curr_ticker = result.loc[
            result["currency_code"] == cur, "ticker"
        ].to_list()
        curr_details = universe_rating_detail.loc[
            universe_rating_detail["ticker"].isin(curr_ticker)
        ]

        # unstack dataframe
        cols = list(set(lst) & set(universe_rating_detail.columns.to_list()))
        curr_details = (
            curr_details.set_index(["ticker"])[cols].unstack().reset_index()
        )
        ...
        curr_des = curr_details.groupby(["factor_name"])["score"].agg(
            ["mean", "std"]
        )
        curr_pos = (
            curr_des["mean"] + 0.4 * curr_des["std"]
        ).to_dict()  # positive = factors > mean + 0.4std
        curr_neg = (
            curr_des["mean"] - 0.4 * curr_des["std"]
        ).to_dict()  # negative = factors < mean - 0.4std
        ...
        for tick in curr_details["ticker"].unique():
            ...
            positive = temp.loc[
                temp["score"] > temp["factor_name"].map(curr_pos)
            ]
            positive = positive.sort_values(by=["score"], ascending=False).head(
                5
            )
            # if len(positive) == 0:
            ...
            negative = temp.loc[
                temp["score"] < temp["factor_name"].map(curr_neg)
            ]
            ...
            for index, row in positive.iterrows():
                positive_factor.append(
                    row["factor_name"]
                )  # positive/negative only first 5 factor
            for index, row in negative.iterrows():
                negative_factor.append(row["factor_name"])
            positive_negative_result = pd.DataFrame(
                {
                    "ticker": [tick],
            ...
            universe_rating_positive_negative = (
                universe_rating_positive_negative.append(
                    positive_negative_result
                )
            )

    universe_rating = rating.merge(universe_rating, how="left", on=["ticker"])
    universe_rating = universe_rating.merge(
        universe_rating_positive_negative, how="left", on=["ticker"]
    )
    ...
    rating_df = pd.DataFrame(
        {"ticker": [], "rating": [], "ai_score": [], "ai_score2": []}, index=[]
    )
    ...
    universe = universe.sort_values(
        by=["ai_score", "ticker"], ascending=[False, True]
    )
    ...
    for cur in factor_use.keys():
        ...
        df_cur = universe_rating.loc[
            universe_rating["ticker"].isin(list(curr_ticker))
        ]
        for i in ["positive_factor", "negative_factor"]:
            df = df_cur.loc[df_cur[i].astype(str) == "[]"]
            if cur in ["USD", "HKD"]:
                if update_firebase:
                    report_to_slack_factor(
                        "*{} : === [{}] without {}: {}/{} ===*".format(
                            dateNow(), cur, i, len(df), len(df_cur)
                        )
                    )
                    report_to_slack_factor(
                        "```"
                        + ", ".join(
                            [
                                "{0:<10}: {1:<5}".format(x, y)
                                for x, y in df[["ticker", "ai_score"]].values
                            ]
                        )
                        + "```"
                    )
    ...
    bot_type = get_bot_type()[
        ["bot_type", "bot_apps_name", "bot_apps_description"]
    ]
    ...
    bot_ranking["duration"] = bot_ranking["duration"].str.replace(
        " ", "-", regex=True
    )
    bot_ranking = bot_ranking.sort_values(by=["ticker", "duration", "ranking"])
    bot_ranking = bot_ranking.drop_duplicates(
        subset=["ticker", "duration", "bot_type"], keep="first"
    )

    bot_statistic = get_bot_statistic_data(
        ticker=ticker, currency_code=currency_code
    )
    bot_statistic = bot_statistic.rename(
        columns={"option_type": "bot_option_type"}
    )
    bot_statistic = bot_statistic[
        [
            "ticker",
            "time_to_exp",
            "lookback",
    ...
    bot_statistic = bot_statistic.loc[
        bot_statistic["time_to_exp"].isin(time_to_exp)
    ]
    bot_statistic = bot_statistic.loc[bot_statistic["lookback"] == 6]
    bot_statistic = bot_statistic.reset_index(inplace=False, drop=True)
    bot_statistic["pct_profit"] = np.where(
        bot_statistic["pct_profit"].isnull(),
        float(0),
        bot_statistic["pct_profit"],
    )
    bot_statistic["avg_profit"] = np.where(
        bot_statistic["avg_profit"].isnull(),
        float(0),
        bot_statistic["avg_profit"],
    )
    bot_statistic["avg_loss"] = np.where(
        bot_statistic["avg_loss"].isnull(), float(0), bot_statistic["avg_loss"]
    )
    ...
    for index, row in bot_statistic.iterrows():
        ...
        bot_statistic.loc[index, "bot_return"] = (
            max(min(avg_profit, 0.5), 0) / 0.5
        )
        bot_statistic.loc[index, "risk_moderation"] = (
            max(0.5 + avg_loss, float(0)) / 0.5
        )
    bot_ranking = bot_ranking.merge(
        bot_statistic[
            [
                "ticker",
                "time_to_exp",
        ...
        how="left",
        on=["ticker", "bot_type", "bot_option_type", "time_to_exp"],
    )
    ...
    for tick in bot_ranking["ticker"].unique():
        ...
        ranking_data = ranking_data.sort_values(
            by=["ticker", "ranking", "duration"]
        )
        ranking_df = []
        for index, row in ranking_data.iterrows():
            rank_data = ranking_data.loc[
                ranking_data["duration"] == row["duration"]
            ]
        ...
        rank = pd.DataFrame(
            {"ticker": [tick], "ranking": [ranking_df]}, index=[0]
        )
    ...
    if update_firebase:
        update_to_firestore(
            data=universe,
            index="ticker",
            table=settings.FIREBASE_COLLECTION["universe"],
            dict=False,
        )
        report_to_slack(
            "{} : === FIREBASE UNIVERSE UPDATED ===".format(datetimeNow())
        )
    else:
        return universe
...
async def do_task(
    ...
    if len(orders_position) > 0:
        ...
        orders_position["current_ivt_amt"] = (
            orders_position["share_num"] * orders_position["price"]
        ).round(rounded)
        orders_position["current_values"] = (
            orders_position["bot_cash_balance"]
            + orders_position["current_ivt_amt"]
        )
        orders_position["current_ivt_amt"] = (
            orders_position["current_ivt_amt"]
            * orders_position["exchange_rate"]
        ).round(2)
        orders_position["current_values"] = (
            orders_position["current_values"] * orders_position["exchange_rate"]
        ).round(2)
        orders_position["bot_cash_balance"] = (
            orders_position["bot_cash_balance"]
            * orders_position["exchange_rate"]
        ...
        # MARGIN
        orders_position["margin_amount"] = (
            orders_position["margin"] - 1
        ) * orders_position["investment_amount"]
        orders_position["bot_cash_balance"] = (
            orders_position["bot_cash_balance"]
            + orders_position["margin_amount"]
        )
        orders_position["threshold"] = (
            orders_position["margin_amount"]
            + orders_position["investment_amount"]
        ) - orders_position["bot_cash_balance"]
        orders_position["margin_amount"] = (
        ...
        orders_position["margin_amount"] = np.where(
            orders_position["margin_amount"] >= 0,
            float(0),
            orders_position["margin_amount"] * -1,
        )

        # PROFIT
        orders_position["profit"] = (
            orders_position["current_values"]
            - orders_position["investment_amount"]
        ).round(rounded)
        orders_position["pct_profit"] = (
            orders_position["profit"]
            / orders_position["investment_amount"]
            * 100
        ).round(2)
        ...
        orders_position["pct_cash"] = np.where(
            orders_position["bot_id"] == "STOCK_stock_0",
            float(0),
            orders_position["pct_cash"],
        )
        ...
        total_invested_amount = float(
            NoneToZero(np.nansum(orders_position["current_values"].to_list()))
        )
        daily_live_profit = float(
            NoneToZero(
                total_invested_amount
                + user_core.loc[0, "pending_amount"]
                - user_core.loc[0, "daily_invested_amount"]
            )
        )
        ...
        pct_total_bot_invested_amount = float(
            NoneToZero(
                round(
                    (total_bot_invested_amount / total_invested_amount) * 100, 2
                )
            )
        )
        pct_total_user_invested_amount = float(
            NoneToZero(
                round(
                    (total_user_invested_amount / total_invested_amount) * 100,
                    2,
                )
            )
        )
        total_profit_amount = NoneToZero(
            np.nansum(orders_position["profit"].to_list())
        )
        ...
        for index, row in orders_position.iterrows():
            act_df = orders_position.loc[
                orders_position["position_uid"] == row["position_uid"]
            ]
            bot = bot_option_type.loc[
                bot_option_type["bot_id"] == row["bot_id"]
            ][
                [
                    "bot_id",
                    "bot_option_type",
            ...
            act_df = act_df.drop(
                columns=["bot_id", "bot_apps_name", "duration"]
            )
            act_df = act_df.to_dict("records")[0]
            active_df.append(act_df)
        total_invested_amount = float(
            formatdigit(total_invested_amount, user_core.loc[0, "is_decimal"])
        )
        total_bot_invested_amount = float(
            formatdigit(
                total_bot_invested_amount, user_core.loc[0, "is_decimal"]
            )
        )
        total_user_invested_amount = float(
            formatdigit(
                total_user_invested_amount, user_core.loc[0, "is_decimal"]
            )
        )
        pct_total_bot_invested_amount = float(
            formatdigit(
                pct_total_bot_invested_amount, user_core.loc[0, "is_decimal"]
            )
        )
        daily_live_profit = float(
            formatdigit(daily_live_profit, user_core.loc[0, "is_decimal"])
        )
        total_profit_amount = float(
            formatdigit(total_profit_amount, user_core.loc[0, "is_decimal"])
        )
        ...
        active = pd.DataFrame(
            {
                ...
                "total_bot_invested_amount": [total_bot_invested_amount],
                "total_user_invested_amount": [total_user_invested_amount],
                "pct_total_bot_invested_amount": [
                    pct_total_bot_invested_amount
                ],
                "pct_total_user_invested_amount": [
                    pct_total_user_invested_amount
                ],
                "total_profit_amount": [total_profit_amount],
                "daily_live_profit": [daily_live_profit],
                "total_portfolio": [total_portfolio],
                "active_portfolio": [active_df],
            },
            index=[0],
        )
    else:
        ...
        active = pd.DataFrame(
            {
                ...
                "total_bot_invested_amount": [zero],
                "total_user_invested_amount": [zero],
                "pct_total_bot_invested_amount": [zero],
                "pct_total_user_invested_amount": [zero],
                "total_profit_amount": [zero],
                "daily_live_profit": [zero],
    ...
    profile = pd.DataFrame(
        {"user_id": [user_core.loc[0, "user_id"]], "profile": [profile]},
        index=[0],
    )
    ...
    result["current_asset"] = (
        result["balance"] + result["total_portfolio"] + result["pending_amount"]
    )
    ...
    result["bot_pending_amount"] = (
        result["bot_pending_amount"].astype(float).round(2)
    )
    result["stock_pending_amount"] = (
        result["stock_pending_amount"].astype(float).round(2)
    )
    result["total_invested_amount"] = (
        result["total_invested_amount"].astype(float).round(2)
    )
    result["total_bot_invested_amount"] = (
        result["total_bot_invested_amount"].astype(float).round(2)
    )
    result["total_user_invested_amount"] = (
        result["total_user_invested_amount"].astype(float).round(2)
    )
    result["total_profit_amount"] = (
        result["total_profit_amount"].astype(float).round(2)
    )
    result["daily_live_profit"] = (
        result["daily_live_profit"].astype(float).round(2)
    )
    if update_firebase:
        await sync_to_async(update_to_firestore)(
            data=result,
            index="user_id",
            table=settings.FIREBASE_COLLECTION["portfolio"],
            dict=False,
        )
    return result


def firebase_user_update(
    user_id=None, currency_code=None, update_firebase=True
):
    ...
    if user_core.empty:
        return
    user_daily_profit = get_user_profit_history(
        user_id=user_id,
        field="user_id, daily_profit, daily_profit_pct, daily_invested_amount, rank::integer, total_profit, total_profit_pct",
    )
    # user_daily_profit["rank"]  = user_daily_profit["rank"].astype(int)
    user_balance = get_user_account_balance(
        currency_code=currency_code,
        user_id=user_id,
        field="user_id, amount as balance, currency_code",
    )
    ...
    user_core["balance"] = np.where(
        user_core["balance"].isnull(), float(0), user_core["balance"]
    )
    user_core.loc[user_core["is_decimal"] == True, "balance"] = round(
        user_core.loc[user_core["is_decimal"] == True, "balance"], 2
    )
    user_core.loc[user_core["is_decimal"] == False, "balance"] = round(
        user_core.loc[user_core["is_decimal"] == False, "balance"], 0
    )
    if len(user_core["user_id"].to_list()) > 0:
        bot_order_pending = get_orders_group_by_user_id(
            user_id=user_core["user_id"].to_list(), stock=False
        )

        user_core = user_core.merge(
            bot_order_pending, how="left", on=["user_id"]
        )
        user_core["bot_pending_amount"] = np.where(
            user_core["bot_pending_amount"].isnull(),
            float(0),
            user_core["bot_pending_amount"],
        )
        stock_order_pending = get_orders_group_by_user_id(
            user_id=user_core["user_id"].to_list(), stock=True
        )
        user_core = user_core.merge(
            stock_order_pending, how="left", on=["user_id"]
        )
        user_core["stock_pending_amount"] = np.where(
            user_core["stock_pending_amount"].isnull(),
            float(0),
            user_core["stock_pending_amount"],
        )
        user_core["pending_amount"] = (
            user_core["stock_pending_amount"] + user_core["bot_pending_amount"]
        )
        user_core["bot_pending_amount"] = user_core[
            "bot_pending_amount"
        ].astype(float)
        user_core["stock_pending_amount"] = user_core[
            "stock_pending_amount"
        ].astype(float)
        user_core["pending_amount"] = user_core["pending_amount"].astype(float)

        orders_position_field = (
            "position_uid, bot_id, ticker, expiry, spot_date, margin"
        )
        orders_position_field += ", entry_price, investment_amount, user_id"
        orders_position_field += (
            ", target_profit_price, max_loss_price, exchange_rate"
        )
        position_data = get_orders_position(
            user_id=user_core["user_id"].to_list(),
            active=True,
            field=orders_position_field,
        )
        position_data["exchange_rate"] = np.where(
            position_data["exchange_rate"].isnull(),
            1,
            position_data["exchange_rate"],
        )
        position_data = position_data.rename(
            columns={
                "target_profit_price": "take_profit",
                "max_loss_price": "stop_loss",
            }
        )
        position_data["investment_amount"] = (
            position_data["investment_amount"] * position_data["exchange_rate"]
        ).round(2)
        # print(position_data)
        position_data["expiry"] = position_data["expiry"].astype(str)
        if len(position_data) > 0:
            universe = get_active_universe(
                ticker=position_data["ticker"].unique()
            )[
                [
                    "ticker",
                    "ticker_name",
            ...
            latest_price = get_price_data_firebase(
                position_data["ticker"].unique().tolist()
            )
            latest_price = latest_price.rename(
                columns={"last_date": "trading_day", "latest_price": "price"}
            )
            position_data = position_data.merge(
                latest_price, how="left", on=["ticker"]
            )
            position_data["price"] = np.where(
                position_data["price"].isnull(),
                position_data["entry_price"],
                position_data["price"],
            )
            position_data["price"] = (
                position_data["price"].astype(float).round(2)
            )
            position_data["trading_day"] = np.where(
                position_data["trading_day"].isnull(),
                position_data["spot_date"],
                position_data["trading_day"],
            )
            position_data = position_data.merge(
                universe, how="left", on=["ticker"]
            )
            position_data["exchange_rate"] = np.where(
                (position_data["exchange_rate"] == 1)
                & (position_data["currency_code"] == "USD"),
                exchange_rate,
                position_data["exchange_rate"],
            )
            ...
            performance_data = get_orders_position_performance(
                position_uid=position_data["position_uid"].to_list(),
                field=orders_performance_field,
                latest=True,
            )
            performance_data["created"] = performance_data["created"].dt.date
            performance_data = performance_data.drop_duplicates(
                subset=["created", "position_uid"], keep="first"
            )
            performance_data = performance_data.drop(columns=["created"])
            position_data = position_data.merge(
                performance_data, how="left", on=["position_uid"]
            )
            position_data = position_data.merge(
                bot_option_type[["bot_id", "bot_apps_name", "duration"]],
                how="left",
                on=["bot_id"],
            )
            position_data["exchange_rate"] = 1
            ...
            position_data["trading_day"] = position_data["trading_day"].astype(
                str
            )
...
def firebase_ranking_update(update_firebase=True):
    rank = get_user_profit_history(
        field="user_id, rank::integer as ranking, rank::integer, total_profit_pct"
    )
    rank = rank.sort_values(by=["rank"], ascending=True).head(6)
    user_core = get_user_core(
        user_id=rank["user_id"].to_list(),
        field="id as user_id, username, current_status, is_joined, first_name, last_name, email",
    )
    user_core = user_core.loc[user_core["current_status"] == "verified"]
    user_core = user_core.loc[user_core["is_joined"] == True]
    ...
    if update_firebase:
        update_to_firestore(
            data=rank,
            index="ranking",
            table=settings.FIREBASE_COLLECTION["ranking"],
            dict=False,
        )
    else:
        return rank
...
def firebase_ranking_update_random(update_firebase=True):
    ...
    for index, row in rank.iterrows():
        rank.loc[index, "total_profit_pct"] = abs(row["total_profit_pct"]) * (
            random.random() * 2
        )
