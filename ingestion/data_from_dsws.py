...
def worldscope_quarter_report_date_from_dsws(ticker = None, currency_code=None, history=False):
    ...
    for period_end in period_end_list:
        result, error_ticker = get_data_history_from_dsws(period_end, period_end, ticker, identifier, filter_field, use_ticker=True, split_number=min(len(universe), 40))
    ...
    data = data.drop(columns=["WC05905A"])
